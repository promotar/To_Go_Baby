...
bool activeDayAppointment(
  ...
  if (DateTime.now().day == day.day && DateTime.now().month == day.month && DateTime.now().year == day.year) {
    if (activeHours
        .where((element) =>
            element.day == day.day &&
    ...
    return (hasRestrictedDay)
        ? !(activeHours.firstWhere(
                (element) => (element.day == day.day &&
                    element.month == day.month &&
                    element.year == day.year &&
                    get(restrictedDays, [day.weekday.toString()], null) != null),
                orElse: () => DateTime(1999)) ==
            DateTime(1999))
        : !(activeHours.firstWhere(
                (element) => (element.day == day.day && element.month == day.month && element.year == day.year),
                orElse: () => DateTime(1999)) ==
            DateTime(1999));
  } else {
    List<int> endOfRange = [0];
    for (int index = 0; index < activeHours.length - 1; index++) {
      if ((activeHours[index + 1].difference(activeHours[index])).inHours > 24) {
...
bool isScheduledDay({
  ...
  for (DateTime time in scheduleTimes) {
    if (time.year == date.year && time.month == date.month && time.day == date.day && date.isAfter(DateTime.now())) {
      return true;
    }
  }
  return false;
