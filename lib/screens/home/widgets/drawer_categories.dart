      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(titleCategory, style: Theme.of(context).textTheme.titleLarge),
          ),
          ...List.generate(
...
class _SubCategoriesState extends State<SubCategories> with Utility {
  ...
  @override
  Widget build(BuildContext context) {
    List<ProductCategory?> categories = widget.productCategory?.categories ?? [].cast<ProductCategory?>();
    ...
    String? name = widget.productCategory?.name ?? "";

    int? count = widget.productCategory?.count ?? 0;

    String? categoryImg = widget.productCategory?.imageUrl("woocommerce_gallery_thumbnail") ?? Assets.noImageUrl;
