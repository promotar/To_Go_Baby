...
class MemberProfile extends StatelessWidget with AppBarMixin, BPGroupMixin {
  ...
  Widget buildRow(
      {required String title, required Widget content, required ThemeData theme, bool enableDivider = true}) {
    return Column(
      children: [
        IntrinsicHeight(
          child: Row(
        ...
        if (member?.profile?.isNotEmpty == true)
          ...member!.profile!.map((e) {
            List<BPMemberProfileField> fields =
                e.fields?.where((element) => element.value?.isNotEmpty == true).toList() ?? [];
