...
abstract class BMChatStoreBase with Store {
  ...
  Future<void> getChat() async {
    if (_conversation?.id != null) {
      final future = _requestHelper
          .getChatBM(id: _conversation?.id ?? 0, queryParameters: {"nocache": DateTime.now().millisecondsSinceEpoch});
  ...
  Future<void> createMessage({dynamic data}) async {
    if (_conversation?.id != null) {
      final future = _requestHelper.sendMessageChatBM(
          id: _conversation?.id ?? 0, data: data, queryParameters: {"nocache": DateTime.now().millisecondsSinceEpoch});
