  ...
  void _handleChat(TranslateType translate) async {
    try {
      ...
      await _settingStore.requestHelper
          .createConversationBM(data: data, queryParameters: {"nocache": DateTime.now().millisecondsSinceEpoch});
      setState(() {
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    List conversations = _send.length == 1
        ? widget.conversations
            .where((e) => (e.participants?.indexWhere((p) => p.userId == _send[0].id) ?? -1) > -1)
            .toList()
        : [];
    return Scaffold(
      appBar: baseStyleAppBar(
        context,
        title:
            widget.enablePrivateMessage ? translate("buddypress_private_message") : translate("bm_create_conversation"),
        actions: [
          SizedBox(
            height: 34,
                        ...
                        children: [
                          Text(
                              translate("bm_current_conversation",
                                  {"name": _send.isNotEmpty == true ? _send[0].name ?? "" : ""}),
