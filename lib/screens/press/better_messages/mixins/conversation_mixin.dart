...
mixin BMConversationMixin {
  ...
  Widget buildUser(
    ...
    if (data?.participants != null && data!.participants!.length < 3) {
      ...
      BMUser? user = data.participants
          ?.firstWhere((element) => element.userId.toString() != authStore.user?.id, orElse: () => BMUser());
  ...
  Widget buildTitle(
    ...
    TextStyle? style = data?.participants != null && data!.participants!.length < 3
